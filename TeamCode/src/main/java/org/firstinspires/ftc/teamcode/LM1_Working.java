...
@TeleOp(name="LM1_Working", group = "LM1")
public class LM1_Working extends OpMode {
    ...
    public void sampleintake() {
        //CR Servo intakes with left trigger and outtakes with right trigger
        if (gamepad2.right_trigger > 0.08) {
            intake_servo.setPower(intake_value);
        }
        else if (gamepad2.left_trigger > 0.08){
        ...
        if (intake_hold == true) {
            ...
            if (intake_motor.getCurrentPosition() > -100 && intake_timer.milliseconds() > 1) {
                intake_motor.setTargetPosition(0);
                intake_motor.setPower(0.2);
                intake_motor.setMode(DcMotor.RunMode.RUN_TO_POSITION);
            }
            else if (intake_motor.getCurrentPosition() < -120 && intake_timer.milliseconds() > 1) {
                intake_motor.setTargetPosition(0);
                intake_motor.setPower(0.4); //abrupt amt of power, get it where it need to be, and then rest
                intake_motor.setMode(DcMotor.RunMode.RUN_TO_POSITION);
            }

            if (intake_timer.milliseconds() > 800 && intake_motor.getPower() > 0) {
                intake_motor.setPower(0);
            }

        }
        else if(intake_hold && intake_motor.getTargetPosition() == -186 && intake_motor.getCurrentPosition() < -120) {
            intake_motor.setPower(0);
        }
